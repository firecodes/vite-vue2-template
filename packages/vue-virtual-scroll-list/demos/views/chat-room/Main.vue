    ...
    <div class="main">
      <div class="list-container">
        <virtual-list
          ...
          @totop="onTotop"
        >
          <div v-show="overflow" slot="header" class="header">
            <div v-show="!finished" class="spinner"></div>
            <div v-show="finished" class="finished">No more data</div>
          </div>
        </virtual-list>
      ...
      getMessages(this.param.pageSize, true).then((messages) => {
        ...
        this.$nextTick(() => {
          ...
          const offset = sids.reduce((previousValue, currentSid) => {
            const previousSize =
              typeof previousValue === 'string' && previousValue !== 0 ? vsl.getSize(previousValue) : previousValue;
