...
export default {
  ...
  computed: {
    ...
    filteredData() {
      const { syntheticActiveFilters } = this;
      const { normalizedColumns } = this;
      ...
      return data
        ? data.filter((row) => {
            for (const columnKey of Object.keys(row)) {
               ...
               */
              const filter =
                columnToFilter.filter === 'default' ? createDefaultFilter(columnKey) : columnToFilter.filter;
              if (columnToFilter && typeof filter === 'function') {
                ...
                } else if (activeFilterOptionValues.some((filterOptionValue) => filter(filterOptionValue, row))) {
                  continue;
                } else {
                  return false;
                }
    ...
    syntheticActiveSorter() {
      ...
      const columnsWithControlledSortOrder = this.normalizedColumns.filter(
        (column) => column.sortOrder === false || column.sortOrder === 'ascend' || column.sortOrder === 'descend',
      );
    ...
    sortedData() {
      ...
      if (activeSorter) {
        ...
        if (
          ...
          (typeof activeSorter.sorter !== 'function' && activeSorter.sorter !== 'default')
        )
          return this.filteredData;
        const filteredData = this.filteredData.slice(0);
        const { columnKey } = activeSorter;
        /**
         * 1 for asc
         * -1 for desc
         */
        const { order } = activeSorter;
        const sorter =
  ...
  created() {
    this.normalizedColumns.forEach((column) => {
      if (column.sorter && (column.defaultSortOrder === 'ascend' || column.defaultSortOrder === 'descend')) {
      ...
      if (column.filter) {
        const { defaultFilterOptionValues } = column;
  ...
  methods: {
    ...
    doUpdateCheckedRowKeys(...args) {
      const { 'onUpdate:checkedRowKeys': onUpdateCheckedRowKeys, onCheckedRowKeysChange } = this;
    ...
    doUpdatePageSize(...args) {
      const { 'onUpdate:checkedRowKeys': onUpdateCheckedKeys, onCheckedRowKeysChange } = this;
