...
export default {
  ...
  computed: {
    ...
    fixedColumnsLeft() {
      ...
      columns.map((column) => {
        ...
        left += this.headerRef.$refs[column.key].offsetWidth;
      });
      return columnsLeft;
    },
    fixedColumnsRight() {
      ...
      for (let i = columns.length - 1; i >= 0; i--) {
        ...
        right += this.headerRef.$refs[columns[i].key].offsetWidth;
      }
      return columnsRight;
    },
  },
  methods: {
    ...
    setActiveRightFixedColumn(target) {
      const { rightFixedColumns } = this.NDataTable;
      const { scrollLeft } = target;
      const { tableWidth } = this;
      const { scrollWidth } = target;
      let rightWidth = 0;
      const { fixedColumnsRight } = this;
    ...
    setActiveLeftFixedColumn(target) {
      const { leftFixedColumns } = this.NDataTable;
      const { scrollLeft } = target;
      let leftWidth = 0;
